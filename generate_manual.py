...
def create_mockup_settlement():
    ...
    # 꿀팁 박스
    draw_bubble_card(d, (150, 440, 1010, 570), "[실특근 정산 및 자동 계산 원리 (v1.43)]",
                     "1. 총 특근일수 = 대체근무 + 법정휴일 + 일반휴일 (대체휴무 제외 표준화)\n"
                     "2. 최종 실특근 = 일반특근 - 사전차감 - (대체휴무 - 대체휴무 시 출장기간 내 사전차감)\n"
                     "3. 사전차감 잔여수 = 총 사전차감수 - 출장기간 내 사전차감수 / 엑셀 보너스 부여(건) 명시")
...
def build_user_presentation(images):
    ...
    p1_tag.text = "SMART OVERTIME SYSTEM v1.43  |  일반 사원 전용 간편 매뉴얼"
    ...
    p1_sub.text = "사원번호 간편 입장, 신청 정보 자동완성, 사용자/관리자 수정 모달 분리 및 개인 달력 조회까지 누구나 쉽게 따라 할 수 있는 사용자 공식 가이드입니다."
    p1_sub.font.size = Pt(15)
    p1_sub.font.color.rgb = RGBColor(203, 213, 225)
    p1_sub.space_after = Pt(28)

    p1_auth = tf1.add_paragraph()
    p1_auth.text = "배포 버전: v1.43 (2026-09-18)  |  PC 모니터 & 스마트폰(모바일) 완벽 지원"
    p1_auth.font.size = Pt(13)
    p1_auth.font.color.rgb = RGBColor(148, 163, 184)

    # --- SLIDE 2: 1단계 - 사번만 넣고 슝 들어가기 ---
    s2_data = [
        ("① v1.43 버전 뱃지 확인", "화면 오른쪽 위에 파란색 [v1.43] 뱃지가 보이면 최신 버전입니다. 클릭 시 신규 릴리즈 이력이 표시됩니다."),
...
def build_admin_presentation(images):
    ...
    p1_tag = tf1.paragraphs[0]
    p1_tag.text = "SMART OVERTIME SYSTEM v1.43  |  관리자 및 운영자 전용 가이드"
    p1_tag.font.size = Pt(14)
    p1_tag.font.bold = True
    p1_tag.font.color.rgb = RGBColor(253, 186, 116)
    p1_tag.space_after = Pt(14)
    ...
    p1_sub = tf1.add_paragraph()
    p1_sub.text = "보너스 수정 지원, 선택 일괄 삭제, 사용자/관리자 수정 모달 분리, 정산표 엑셀 수식 최적화까지 완벽 지원하는 관리자 공식 가이드입니다."
    p1_sub.font.size = Pt(15)
    p1_sub.font.color.rgb = RGBColor(203, 213, 225)
    p1_sub.space_after = Pt(28)

    p1_auth = tf1.add_paragraph()
    p1_auth.text = "배포 버전: v1.43 (2026-09-18)  |  총괄 슈퍼관리자 및 부서 팀관리자 전용"
    p1_auth.font.size = Pt(13)
    p1_auth.font.color.rgb = RGBColor(148, 163, 184)
    ...
    s6_data = [
        ("① 최신 실특근 산정 공식 (v1.43)", "★ 총특근일수 = 대체근무 + 법정휴일 + 일반휴일 (대체휴무 제외 표준화)\n★ 최종 실특근일 = 일반특근 - 사전차감 - (대체휴무 - 대체휴무시 출장기간 내 사전차감)"),
    ...
    p_title10.text = "자주 묻는 질문(Q&A)과 관리자 운영 꿀팁 총정리 (v1.43)"
    ...
    admin_qa_items = [
        ...
        ("Q5. 서버가 재부팅되어도 인원과 특근 데이터가 영구 보존되나요?", "네! Turso 클라우드 영구 DB 연동으로 모든 데이터가 안전 보존되며, v1.43의 안전 스마트 폴백과 스레드 보호로 특근 신청과 데이터 무결성을 완벽하게 보장합니다.")
    ]
