...
# ----------------- 지시선 및 사용자용 친절한 말풍선 핀 헬퍼 (v1.44 고해상도 & 고시인성) -----------------

def draw_browser_frame(d, title="★ 스마트 특근 관리 시스템 (v1.44)", url="https://overtime-system.internal"):
    """
    ...
    d.text((1505, 50), "v1.44 최신", font=get_font(16, bold=True), fill="#ffffff", anchor="mm")
...
def create_mockup_login():
    ...
    # 스마트 지시선 배치 (충돌 0%)
    draw_smart_pin(d, (1505, 70), (1350, 125), "①", "v1.44 최신 버전 배지", color="#0284c7", sub_hint="최신 릴리즈 확인")
...
def create_mockup_settlement():
    ...
    img = Image.new("RGB", (CANVAS_W, CANVAS_H), "#f8fafc")
    d = ImageDraw.Draw(img)
    draw_browser_frame(d, title="★ 스마트 특근 관리 시스템  |  실특근 정산 및 자동 계산 (v1.44)", url="https://overtime.company.com/admin/settlement")

    d.rounded_rectangle([40, 105, 1560, 1060], radius=16, fill="#ffffff", outline="#cbd5e1", width=2)

    # 공식 배너
    d.rounded_rectangle([80, 125, 1520, 235], radius=12, fill="#ecfdf5", outline="#10b981", width=3)
    d.text((115, 145), "★ 컴퓨터가 1초 만에 자동 산출하는 진짜 일한 날(최종 실특근) 공식! (v1.43 개편)", font=get_font(22, bold=True), fill="#065f46")
...
def build_user_presentation(images):
    ...
    p1_tag.text = "SMART OVERTIME SYSTEM v1.44  |  일반 사원 전용 공식 매뉴얼"
    ...
    p1_sub.text = "특근 완료 확정 피드백(3단계), 100% 무기명 건의 소통함, 접속 주소 및 접속 방법, 서버 초기화 대기 안내, 사원번호 간편 입장, 스마트 달력 신청까지 누구나 쉽게 따라 할 수 있는 사용자 공식 가이드입니다."
    p1_sub.font.size = Pt(15)
    p1_sub.font.color.rgb = RGBColor(203, 213, 225)
    p1_sub.space_after = Pt(24)

    p1_auth = tf1.add_paragraph()
    p1_auth.text = "배포 버전: v1.44 (2026-09-21)  |  PC 모니터 & 스마트폰(모바일) 완벽 지원"
    ...
    s4_data = [
        ("① v1.44 최신 버전 배지 확인", "화면 오른쪽 위에 파란색 [v1.43] 배지가 보이면 최신 버전입니다. 클릭 시 신규 릴리즈 이력이 표시됩니다."),
    ...
    # --- SLIDE 8: 7단계 - 4단계 특근 라이프사이클 & 특근 완료 후 [확정] 피드백 (v1.44 신규) ---
    ...
    lifecycle_cards = [
        ("1. 특근 4단계 라이프사이클 체계",
         ...
         "• 2단계 [승인]: 부서 관리자가 계획을 승인한 상태\n"
         "• 3단계 [확정]: 실제 휴일 특근을 수행 완료했음을 피드백\n"
         "• 4단계 [검토완료]: 관리자가 최종 마감 처리한 공인 상태",
         "#eff6ff", "#2563eb"),
        ("2. 사원의 [🎯 특근완료 확정] 피드백",
         "• 주말/휴일 특근을 실제로 마치면 '내 신청 내역'으로 이동합니다.\n"
         "• 승인된 특근 카드 우측에 나타나는 [🎯 특근완료 확정] 버튼을 클릭!\n"
         "• '실제 특근을 완료하여 확정하시겠습니까?' 확인 시 즉시 확정 상태로 전환됩니다.",
         "#ecfdf5", "#10b981"),
        ("3. 진행상태 배지 & 최종특근일 반영",
         "• 확정 즉시 카드 상단에 주황색 [🎯 3단계:확정] 배지가 부여됩니다.\n"
         "• 관리자 화면 및 정산 엑셀 파일의 [확정단계 (일)]에 실시간 반영되어 정산 누락을 방지합니다.\n"
         "• 관리자의 최종 검토가 끝나면 [🟣 4단계:검토완료]로 최종 종결됩니다.",
         "#fef3c7", "#ea580c")
    ]
    ...
    # --- SLIDE 9: 8단계 - 사원 전용 100% 무기명 건의사항 소통함 (v1.44 신규) ---
    ...
    p9_step.text = "STEP 08  |  사원 전용 100% 무기명 건의사항 소통함 (v1.44 신규)"
    ...
    p_step11.text = "SUMMARY & Q&A  |  사용자 자주 묻는 질문 답변 (v1.44)"
...
def build_admin_presentation(images):
    ...
    p1_tag = tf1.paragraphs[0]
    p1_tag.text = "SMART OVERTIME SYSTEM v1.44  |  관리자 및 운영자 전용 가이드"
    p1_tag.font.size = Pt(14)
    p1_tag.font.bold = True
    p1_tag.font.color.rgb = RGBColor(253, 186, 116)
    p1_tag.space_after = Pt(14)
    ...
    p1_sub = tf1.add_paragraph()
    p1_sub.text = "특근 4단계 라이프사이클(신청·승인·확정·검토), 엑셀 단계별 일수 구분 집계, 100% 무기명 소통함 운영까지 완벽 지원하는 관리자 공식 가이드입니다."
    p1_sub.font.size = Pt(15)
    p1_sub.font.color.rgb = RGBColor(203, 213, 225)
    p1_sub.space_after = Pt(28)

    p1_auth = tf1.add_paragraph()
    p1_auth.text = "배포 버전: v1.44 (2026-09-21)  |  총괄 슈퍼관리자 및 부서 팀관리자 전용"
    p1_auth.font.size = Pt(13)
    p1_auth.font.color.rgb = RGBColor(148, 163, 184)
    ...
    # --- SLIDE 6: 실특근 자동 산정 및 정산표 (v1.43 개편) ---
    s6_data = [
        ("① 최신 실특근 산정 공식 (v1.44)", "★ 총특근일수 = 대체근무 + 법정휴일 + 일반휴일 (대체휴무 제외 표준화)\n★ 최종 실특근일 = 일반특근 - 사전차감 - (대체휴무 - 대체휴무시 출장기간 내 사전차감)"),
    ...
    # --- SLIDE 8: 특근 4단계 검증 체계 및 관리자 [검토완료] (v1.44 신규) ---
    ...
    p8_step_adm.text = "STEP 07  |  특근 4단계 검증 체계 및 [검토완료] 마감 (v1.44 신규)"
    ...
    p8_title_adm.text = "특근 4단계 검증 체계 및 관리자 [검토완료] 최종 마감"
    ...
    p8_sub_adm.text = "사원의 확정 피드백 검증, [검토완료] 원클릭 마감 및 상단 [일괄 확정/검토] 강력 지원"
    p8_sub_adm.font.size = Pt(13)
    p8_sub_adm.font.color.rgb = COLOR_TEXT_MUTED

    admin_stage_cards = [
        ("1. 4단계 라이프사이클 통제",
         "• 1단계(신청:회색), 2단계(승인:청색), 3단계(확정:주황), 4단계(검토완료:보라) 컬러 배지로 상태 직관 파악\n"
         "• 테이블 검색 필터에서 4단계를 각각 선택하여 대기/완료 현황을 1초 만에 필터링 조회합니다.\n"
         "• 미니 통계 칩에 신청, 승인, 확정, 검토완료 수량이 실시간 집계됩니다.",
         "#eff6ff", "#2563eb"),
        ("2. 건별 [🎯 확정] 및 [🟣 검토완료]",
         "• 관리자가 사원을 대신해 직접 [🎯 확정] 버튼을 눌러 피드백을 처리할 수 있습니다.\n"
         "• 사원 또는 관리자가 확정한 건에 대해 [🟣 검토완료] 버튼을 클릭해 최종 검토 마감 처리합니다.\n"
         "• 검토완료 시 검토자 성명과 일시(예: 홍길동(09-21 14:00))가 안전 기록됩니다.",
         "#ecfdf5", "#10b981"),
        ("3. 상단 [일괄 확정] & [일괄 검토완료]",
         "• 관리자 테이블 목록에서 체크박스로 여러 특근 건을 복수 선택합니다.\n"
         "• 상단의 [🎯 일괄 확정] 또는 [🟣 일괄 검토완료] 버튼을 눌러 대량의 특근을 한 번에 마감!\n"
         "• 월말 및 분기별 대규모 정산 시 소요 시간을 90% 이상 단축합니다.",
         "#fef3c7", "#ea580c")
    ]
    ...
    # --- SLIDE 9: 특근정보 엑셀 내보내기 4단계 구분 집계 (v1.44 신규) ---
    ...
    p9_step_excel.text = "STEP 08  |  특근정보 엑셀 내보내기 4단계 구분 집계 (v1.44 신규)"
    ...
    excel_stage_cards = [
        ("1. Sheet 1 [휴일일자별_특근현황]",
         "• 4단계 진행상태 열(신청:회색, 승인:청색, 확정:주황, 검토완료:보라) 컬러 스타일 적용\n"
         "• 승인자, 확정자, 검토완료자 일시 정보가 정확하게 기록됩니다.\n"
         "• 특정 일자에 누가 확정하고 검토했는지 한눈에 추적 가능합니다.",
         "#eff6ff", "#2563eb"),
        ("2. Sheet 2 [개인별_휴일합산_정산표]",
         "• '신청단계 (일)', '승인단계 (일)', '확정단계 (일)', '검토완료 (일)' 4개 열로 완전 분리 집계!\n"
         "• 각 단계별 일수 합계(SUM 수식)가 하단에 자동 계산됩니다.\n"
         "• 확정 및 검토완료된 일수만 신속하게 발췌하여 급여에 반영할 수 있습니다.",
         "#ecfdf5", "#10b981"),
        ("3. Sheet 4 [부서별_요약] 및 정산표",
         "• 부서별 요약 시트에도 4단계별 총 일수가 자동 집계되어 부서 간 진행 상황을 비교합니다.\n"
         "• 관리자 정산표 모달 및 정산 엑셀 파일에도 동일한 4단계 분리 컬럼이 지원됩니다.\n"
    ...
    # --- SLIDE 10: 사원 무기명 건의사항 소통함 운영 및 공식 답변 (v1.44 신규) ---
    ...
    p10_step_sug.text = "STEP 09  |  사원 무기명 건의사항 소통함 운영 & 답변 (v1.44 신규)"
    ...
    p_title10.text = "자주 묻는 질문(Q&A)과 관리자 운영 꿀팁 총정리 (v1.44)"
    ...
    admin_qa_items = [
        ...
        ("Q5. 서버가 재부팅되어도 인원과 특근 데이터가 영구 보존되나요?", "네! Turso 클라우드 영구 DB 연동으로 모든 데이터가 안전 보존되며, v1.44의 안전 스마트 폴백과 스레드 보호로 특근 신청과 데이터 무결성을 완벽하게 보장합니다."),
        ("Q6. 특근 4단계 라이프사이클에서 관리자의 [검토완료]는 어떤 의미인가요?", "사원 및 관리자가 실제 특근 완료를 [확정]한 후, 관리자가 급여/노무 정산 직전 최종 검토하여 마감 처리했음을 공인하는 최종 단계입니다."),
